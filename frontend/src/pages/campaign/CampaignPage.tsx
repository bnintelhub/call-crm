...
export const CampaignPage: React.FC = () => {
  ...
  const { updateAllocation, allocationsList } = useAllocationStore();
  const { campaignsList, addCampaign, updateCampaign: updateCamp, deleteCampaign } = useCampaignStore();
  ...
  const routeState = location.state as { createFromAllocation?: any };
  const initialCreateAllocation = routeState?.createFromAllocation;

  // Modals state
  const [isCreateModalOpen, setIsCreateModalOpen] = useState(!!initialCreateAllocation);
  const [preSelectedAllocation, setPreSelectedAllocation] = useState<string | null>(
    initialCreateAllocation ? initialCreateAllocation.id : null
  );
  ...
  // Filter campaigns belonging to current allocator
  const allocatorCampaignsList = useMemo(() => {
    if (!allocator) return campaignsList; // If no allocator selected, maybe return all or none? Usually return all.
    
    // Find all allocations for this allocator
    const companyAllocations = allocationsList.filter(item => 
      item.allocationName.toLowerCase().startsWith(allocator.toLowerCase())
    );
    const companyAllocationIds = new Set(companyAllocations.map(a => a.id));

    return campaignsList.filter(camp => {
      // If linked to an allocation, check if it belongs to the allocator
      if (camp.allocationId) {
        return companyAllocationIds.has(camp.allocationId);
      }
      // Fallback for demo data: check if campaign name starts with allocator name
      return camp.name.toLowerCase().startsWith(allocator.toLowerCase());
    });
  }, [campaignsList, allocationsList, allocator]);

  // Stats calculation
  const totalCampaigns = allocatorCampaignsList.length;
  const activeCampaigns = allocatorCampaignsList.filter((c) => c.status === 'Running').length;
  const totalAgents = allocatorCampaignsList.reduce((acc, c) => acc + (c.agentsCount || 0), 0);
  const onlineAgents = Math.round(totalAgents * 0.7);
  const totalCompletedAutodial = allocatorCampaignsList.reduce((acc, c) => acc + (c.completedAutodial || 0), 0);

  // Category counts
  const categoryCounts = useMemo(() => {
    return {
      all: allocatorCampaignsList.length,
      predictive: allocatorCampaignsList.filter((c) => c.category === 'PREDICTIVE').length,
      progressive: allocatorCampaignsList.filter((c) => c.category === 'PROGRESSIVE').length,
      manual: allocatorCampaignsList.filter((c) => c.category === 'MANUAL').length,
      preview: allocatorCampaignsList.filter((c) => c.category === 'PREVIEW').length,
    };
  }, [allocatorCampaignsList]);

  // Filtered campaigns
  const filteredCampaigns = useMemo(() => {
    return allocatorCampaignsList.filter((campaign) => {
      // 1. Category Filter
  ...
  }, [allocatorCampaignsList, activeCategory, statusFilter, searchQuery]);
  ...
  const handleCampaignCreated = (newCamp: CampaignItem) => {
    addCampaign(newCamp);
    if (newCamp.allocationId) {
      updateAllocation(newCamp.allocationId, { tabCategory: '100% Allocated' });
    }
  ...
  const handleCampaignUpdated = (updatedCamp: CampaignItem) => {
    updateCamp(updatedCamp.id, updatedCamp);
  ...
  const handleAction = (
    ...
    if (action === 'view') {
      ...
      updateCamp(campaign.id, { status: nextStatus });
    ...
    <div className="campaign-page-container">
      ...
      <CreateCampaignModal
        ...
        onClose={() => {
          ...
          // clear route state so it doesn't re-open on refresh
          window.history.replaceState({}, document.title);
        }}
        onCampaignCreated={handleCampaignCreated}
        initialAllocationId={preSelectedAllocation}
      />
