...
export default function SuperAdminDashboard() {
  ...
  const seatsSold = companies.reduce((n, c) => n + (c.quotas?.seats || 0), 0);
  const seatsUsed = companies.reduce((n, c) => n + (c.usage?.seatsUsed || 0), 0);
  const expiring = companies
    .filter((c) => c.status === 'active' || c.status === 'trial' || c.status === 'grace')
    .map((c) => ({ ...c, left: c.endDate ? daysUntil(c.endDate) : 0 }))
    .filter((c) => c.left <= 7)
