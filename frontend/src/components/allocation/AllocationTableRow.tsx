...
interface AllocationTableRowProps {
  ...
  onCreateCampaignClick?: (item: AllocationItem) => void;
}
...
export const AllocationTableRow: React.FC<AllocationTableRowProps> = ({
  ...
  return (
    <tr className="alloc-table-row">
      ...
      {/* 10. Actions / Payment file status */}
      <td className="alloc-cell-action" style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
        {item.tabCategory === 'Unallocated' && onCreateCampaignClick ? (
          <button
            type="button"
            className="alloc-upload-action-btn"
            style={{ backgroundColor: 'var(--accent-primary)', color: 'white', border: 'none' }}
            onClick={() => onCreateCampaignClick(item)}
          >
            Create Campaign
          </button>
        ) : null}
